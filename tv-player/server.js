...
function loadState() {
  try {
    ...
    if (!fs.existsSync(STATE_FILE)) {
      fs.writeFileSync(STATE_FILE, JSON.stringify(def, null, 2));
      return def;
    }
    ...
    console.error('[state] load failed, using default:', e.message);
    return defaultState();
  }
}
...
async function collectChannels() {
  ...
  // Emergency fallback: if everything failed (Render network block), always return verified raw
  if (channels.length === 0) {
    try {
      const fallback = parseM3U(VERIFIED_RAW, 'verified', '✅ Verified Working — Emergency Fallback');
      channels.push(...fallback.map(c => ({ ...c, source: '✅ Emergency Fallback', sourceId: 'verified' })));
      sources.push({ id: 'verified', name: '✅ Emergency Fallback', reachable: true, count: fallback.length });
    } catch (e) {}
  }
  const categories = {};
...
function validFtpUrl(u) {
  try {
    const parsed = new URL(u);
    if (parsed.protocol !== 'ftp:' && parsed.protocol !== 'ftps:') return false;
    const h = parsed.hostname.toLowerCase();
    if (
      h === 'localhost' ||
      h.endsWith('.localhost') ||
...
// FTP -> HTTP streaming proxy (passive mode, zero deps)
// Supports ftp://user:pass@host/path and anonymous ftp
function parseFtpUrl(s) {
  const u = new URL(s);
  return {
    host: u.hostname,
    port: u.port ? parseInt(u.port, 10) : 21,
    user: u.username ? decodeURIComponent(u.username) : 'anonymous',
    pass: u.password ? decodeURIComponent(u.password) : 'anonymous@',
    path: (u.pathname || '/') + (u.search || ''),
  };
}
function ftpPasvToEndpoint(txt) {
  // 227 Entering Passive Mode (192,168,1,1,15,10)
...
function ftpPipe(res, ftpUrlStr) {
  return new Promise((resolve, reject) => {
    let info;
    try { info = parseFtpUrl(ftpUrlStr); } catch (e) { res.writeHead(400, { 'content-type': 'text/plain' }); res.end('bad ftp url'); return resolve(); }
    const ctrl = net.createConnection({ host: info.host, port: info.port }, () => {});
    let buf = '';
    let stage = 'connect';
    let dataHost = null, dataPort = null;
    let dataSock = null;
    let fileName = path.basename(info.path) || 'file';
    const ext = path.extname(fileName).toLowerCase();
    const mimeMap = { '.mp4': 'video/mp4', '.mkv': 'video/x-matroska', '.avi': 'video/x-msvideo', '.mov': 'video/quicktime', '.m3u8': 'application/vnd.apple.mpegurl', '.ts': 'video/mp2t', '.mp3': 'audio/mpeg' };
    const contentType = mimeMap[ext] || 'application/octet-stream';

    const clean = () => {
      try { ctrl.destroy(); } catch (e) {}
      try { if (dataSock) dataSock.destroy(); } catch (e) {}
    };
    const fail = (code, msg) => {
      if (!res.headersSent) res.writeHead(code, { 'content-type': 'text/plain', 'access-control-allow-origin': '*' });
      res.end(msg);
      clean();
      resolve();
    };
    const send = (cmd) => new Promise((r) => {
      ...
      const onData = (chunk) => {
        ...
        let lines = buf.split('\r\n');
        // wait for complete reply (code + space)
    ...
    ctrl.setTimeout(20000, () => fail(504, 'FTP control timeout'));
    ctrl.on('error', (e) => fail(502, 'FTP control error: ' + e.message));

    // initial banner
    let banner = '';
    ctrl.on('data', function bannerHandler(chunk) {
      banner += chunk.toString('utf8');
      if (banner.includes('\r\n')) {
        ctrl.removeListener('data', bannerHandler);
        // start login sequence
        (async () => {
          try {
            let resp = await send(`USER ${info.user}`);
            if (resp.startsWith('331')) {
              resp = await send(`PASS ${info.pass}`);
            }
            if (!resp.startsWith('2')) {
              return fail(502, 'FTP login failed: ' + resp.slice(0, 300));
            }
            resp = await send('TYPE I');
            // passive mode
            resp = await send('PASV');
            const ep = ftpPasvToEndpoint(resp);
            if (!ep) return fail(502, 'FTP PASV parse failed: ' + resp.slice(0, 300));
            dataHost = ep.ip; dataPort = ep.port;

            // prepare data connection BEFORE RETR
            const dataPromise = new Promise((resData, rejData) => {
              dataSock = net.createConnection({ host: dataHost, port: dataPort }, () => resData());
              dataSock.setTimeout(30000, () => rejData(new Error('FTP data timeout')));
              dataSock.on('error', rejData);
            });
            ...
            // retrieve file
            const retrPromise = send(`RETR ${info.path}`);
            await dataPromise; // data connection ready

            // pipe data sock to http res
            dataSock.on('data', (chunk) => {
              try { res.write(chunk); } catch (e) {}
            });
            dataSock.on('end', async () => {
              try {
                const finalResp = await retrPromise;
                // finalResp should be 226
                res.end();
              } catch (e) {
                res.end();
              }
            ...
            dataSock.on('error', (e) => {
              fail(502, 'FTP data error: ' + e.message);
            });

            // send RETR already sent, wait for 150 reply
            const retrResp = await retrPromise;
            if (!retrResp.startsWith('1') && !retrResp.startsWith('2')) {
              // if RETR failed, try to read more
              return fail(502, 'FTP RETR failed: ' + retrResp.slice(0, 400));
            }
            // if server already finished (small file) and data ended, handle
          } catch (e) {
            fail(502, 'FTP error: ' + (e.message || e));
          }
        })();
      }
    });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ----- API: FTP streaming proxy (functional FTP) -----
    if (p === '/api/ftp-stream' && req.method === 'GET') {
      const target = u.searchParams.get('url');
      if (!target || !validFtpUrl(target)) {
        res.writeHead(400, { 'content-type': 'text/plain', 'access-control-allow-origin': '*' });
        return res.end('bad ftp url - must be ftp://...');
      }
      return ftpPipe(res, target).catch((e) => {
        if (!res.headersSent) res.writeHead(502, { 'content-type': 'text/plain', 'access-control-allow-origin': '*' });
        res.end('ftp proxy error: ' + (e.message || e));
      });
    }
